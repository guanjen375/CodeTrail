...
每項都綁定加入時的 session 與 turn。選擇框停留到原回合結束後才選「補充」，會改為待下一輪，不會插入另一個正在執行的任務。尚有未送項目時，切換/新建對話、手動壓縮與正常退出會提示先處理佇列；可送完或逐項取消。`/resume <session id>` 仍是接續對話，與 `/queue resume` 不同。
