...
| 已啟用的 [DSpark](dspark.md) | 配對的本地 draft GGUF（含完整 shards）與支援 `draft-dspark` 的 llama-server。設定與啟動驗證相依性；啟動、host readiness 與 strict status 必須由 `/slots` 確認真正啟用，缺席／失敗不退回普通解碼後宣稱成功。關閉不要求 draft 或 DSpark binary 能力。 |
