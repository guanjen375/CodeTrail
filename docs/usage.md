...
以滑鼠拖選對話文字，串流中的回答、完成的回答與接續歷史後重播的回答都可選取。
完成與重播的回答保留 Markdown 標題、清單及程式碼上色。

| 按鍵與情境 | 行為 |
|---|---|
| **目前複製鍵（預設 F2）** | 複製目前畫面的選取文字；忙碌、核准框或其他選單開著時也可用，不會中斷、核准或關閉畫面 |
...
目前複製鍵同樣優先取目前畫面的文字選取，其次取目前焦點輸入框的選取，不會取被彈出視窗
遮住的對話選取。複製後保留選取，可重複複製；要用 Ctrl+C 離開，先取消選取。
沒有選取時複製鍵不清空剪貼簿。提示「已送出複製請求」只代表請求已送出，請在本機
貼上確認內容；SSH、tmux 與終端支援的設定見[剪貼簿排查](../developer.md#clipboard-ssh-tmux)。


用 `/copykey` 查看目前鍵與可用鍵，`/copykey f3` 立即改成 F3，`/copykey reset` 回 F2。
...
底部常駐「滑鼠拖曳選取後按目前鍵複製 · /copykey 修改」。更換複製鍵不改 Ctrl+C 的中斷行為。
