...
| 按鍵與情境 | 行為 |
|---|---|
| **F2** | 複製目前畫面的選取文字；忙碌、核准框或其他選單開著時也可用，不會中斷、核准或關閉畫面 |
...
F2 同樣優先取目前畫面的文字選取，其次取目前焦點輸入框的選取，不會取被彈出視窗
遮住的對話選取。複製後保留選取，可重複複製；要用 Ctrl+C 離開，先取消選取。
沒有選取時 F2 不清空剪貼簿。提示「已送出複製請求」只代表請求已送出，請在本機
貼上確認內容；SSH、tmux 與終端支援的設定見[剪貼簿排查](troubleshooting.md#clipboard-ssh-tmux)。
