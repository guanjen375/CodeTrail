...
本工具同時支援 RAG（`--kb`）和 MCP（`--mcp`）兩種資料取用方式，適用場景不同：

| | RAG（`--kb`） | MCP（`--mcp`） |
|--|--|--|
| **誰決定讀什麼** | 你決定 — 預先把文件建成知識庫 | 模型決定 — 自己去遠端翻 |
| **資料搬運** | 預先處理：切 chunk、算 embedding、存成 JSON | 不搬運：透過 SSH 按需讀取 |
| **適合場景** | 資料量可控、你確定這些文件都很重要 | 資料量大、不確定哪些相關 |
...
兩者可以混用 — `--mcp` 負責探索遠端程式碼，`--kb` 負責提供你確定重要的文件參考：
...
透過 SSH 按需存取遠端主機上的檔案，模型自行決定要讀取哪些檔案、搜尋什麼內容：
