...
_HIGH_PRIORITY_TOOLS = {
    "read_file", "grep", "find", "list_files",
}
...
def trim_tool_message(
    ...
    marker = (
        ...
        f"original_chars={len(content)} tool={tool_name or 'unknown'} "
        "reason=budget\n"
    )
    if len(marker) >= max_chars:
        result = marker[:max_chars]
    else:
        payload_budget = max_chars - len(marker)
        head_budget = payload_budget // 3
        tail_budget = payload_budget - head_budget
        tail = content[-tail_budget:].lstrip("\n") if tail_budget else ""
        result = content[:head_budget].rstrip() + marker + tail
