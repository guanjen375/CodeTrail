...
import opencode_migrate  # noqa: E402
GIB = 1024**3
MIB = 1024**2
...
def _is_foreign_opencode_target(target: Path, state_target: Path) -> bool:
    """這個 restore 目標是不是「別人的 OpenCode 設定」?

    升級前的 transaction manifest 仍列著 `~/.config/opencode/opencode.json`
    與 OpenCode 的壓縮 ownership 狀態檔。CodeTrail 現在完全不寫那份設定,
    唯一還會碰它的是 `opencode_migrate`(只還原現值仍等於我們寫入值的鍵)。
    `--restore-last-backup` 照舊 manifest 寫回去會整份覆蓋,繞過 ownership。
    """
    if target == state_target:
        # ownership 狀態檔只跟 opencode.json 一起才有意義:單獨把它寫回去,
        # 就是在描述一份我們已經不再接管的設定。兩個一起跳過。
        return True
    if "opencode" in target.parts:
        return True
    return target.name == "opencode.json"


def _migration_env(home: Path) -> dict[str, str]:
    """遷移要看的 env。

    只帶 `HOME` 會把使用者的 `OPENCODE_CONFIG` 丟掉 —— 那樣 plan 看的是
    `~/.config/opencode/opencode.json`(可能不存在),卻仍然刪掉唯一那份
    ownership 狀態檔,自訂設定裡我們寫進去的值從此還原不回來。
    """
    env = {"HOME": str(home)}
    for key in ("OPENCODE_CONFIG", "XDG_CONFIG_HOME", "USERPROFILE"):
...
def restore_last_backup(home: Path, dry_run: bool = False) -> int:
    ...
    if manifest_file.is_file():
        ...
        if targets:
            ...
            skipped_foreign: list[Path] = []
            for target_str, info in targets.items():
                target = Path(target_str)
                # 升級前的 manifest 還列著 opencode.json / OpenCode 的
                # compaction 狀態檔。CodeTrail 已經不寫那份設定了,
                # `opencode_migrate` 是唯一還會碰它的路徑(而且只還原
                # 現值仍等於我們寫入值的鍵)。照舊 manifest 寫回去會繞過
                # 那層 ownership 語意,把使用者現在的設定換成一份舊備份。
                if _is_foreign_opencode_target(target, state_target):
                    skipped_foreign.append(target)
                    continue
            ...
            for target in skipped_foreign:
                print(
                    f"  - 跳過 {target}(舊 manifest 的 OpenCode 設定;"
                    "還原它要用 opencode_migrate,不是這條路徑)"
                )
            if problems:
...
def _print_summary_page(plan: Plan, python_bin: str,
    ...
    if migration_plan is not None and migration_plan.needed:
        print("  舊 OpenCode 設定遷移(寫入之後執行,有備份):")
        for line in migration_plan.render():
            print(f"    {line}")
...
def run(args: argparse.Namespace) -> int:
    ...
    # 舊 OpenCode 安裝的一次性遷移。**只在偵測到 CodeTrail 自己寫過的東西時**
    # 才會動使用者的 opencode.json;沒有狀態檔、也沒有我們的 plugin 項的機器
    # 一個 byte 都不會被碰。走既有 ownership 語意與備份。
    migration_env = _migration_env(home)
    try:
        migration_plan = opencode_migrate.plan_migration(migration_env)
    except opencode_migrate.MigrationError as exc:
        # 判不出「要不要遷移」就不能寫任何東西:ownership 狀態檔壞掉、綁到別份
        # config、OpenCode 設定解析不了——舊值與 plugin 項可能還在使用者的設定裡
        # 沒人管。降成一則 note 然後 exit 0,等於把這件事藏起來。
        print(f"[FAIL] 舊 OpenCode 設定的遷移狀態無法判定:{exc}", file=sys.stderr)
        print(
            "[set_config] 未寫入任何檔案。請先處理上面的原因"
            "(python3 opencode_migrate.py --check 可以單獨看),再重跑。",
            file=sys.stderr,
        )
        return 2
    ...
    def _gather(notes: list[str]) -> dict:
        ...
        if chosen_mode is None:
            ...
            if chosen_mode != client_compaction.MODE_OFF:
                ...
                # → compaction_mode.derive_settings)。這裡先算一次只為了在摘要頁把
            ...
            settings = client_config.ClientSettings(
                ...
                permission=dict(prior_client_settings.permission),
            )
    ...
    # 遷移在設定寫完之後才做:它動的是**別人的**設定(使用者的 opencode.json),
    # 所以只在這次真的有寫入時才執行,而且自己有備份與 transaction。
    migration_failed = ""
    if migration_plan is not None and migration_plan.needed and not args.dry_run:
        try:
            applied = opencode_migrate.apply_migration(migration_env)
        except opencode_migrate.MigrationError as exc:
            migration_failed = str(exc)
        else:
            print("\n=== 舊 OpenCode 設定遷移 ===")
            for line in applied.render():
                print(f"  {line}")
    ...
    print("\n=== 結果 ===")
    if migration_failed:
        # CodeTrail 自己的設定已經寫進去了,但使用者的 OpenCode 設定沒還原成功。
        # 印 [PASS] 然後 exit 0 等於把這件事藏起來:那台機器每次啟動都會再被
        # 提示遷移,而使用者以為已經處理完了。
        print(f"[FAIL] 舊 OpenCode 設定遷移失敗:{migration_failed}", file=sys.stderr)
        print(
            "[set_config] CodeTrail 自己的設定已寫入並通過驗證;"
            "請處理上面的原因後單獨重跑 `python3 opencode_migrate.py`。",
            file=sys.stderr,
        )
        return 2
    if args.dry_run:
